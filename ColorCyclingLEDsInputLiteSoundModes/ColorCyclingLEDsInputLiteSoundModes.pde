...
void draw() {
  if (showStats) {
    ...
    stats += "\nSOUND_ENERGY mode: " + beatDetectSoundEnergyMode;
  ...
  if (!allPaused) 
  {
    ...
    lowAmplitude = fftLin.getAvg(0) * soundSensitivity;
    midAmplitude = fftLin.getAvg(1) * soundSensitivity;
    highAmplitude = fftLin.getAvg(2) * soundSensitivity;
